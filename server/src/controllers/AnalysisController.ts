...
export class AnalysisController {
  ...
  ask = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const answer = await this.analysisService.ask(fileId, req.user.userId, question, history);
  ...
  report = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const report = await this.analysisService.generateReport(fileId, req.user.userId);
